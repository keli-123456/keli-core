...
func (s *userStore) AddUser(_ context.Context, user *protocol.MemoryUser) error {
	account, err := socksAccountFromUser(user)
	if err != nil {
		return err
	}
...
func (s *Server) AddUser(ctx context.Context, user *protocol.MemoryUser) error {
	...
	if err := s.users.AddUser(ctx, user); err != nil {
		return err
	}
	if s.httpServer == nil {
		return nil
	...
	httpUser := cloneMemoryUser(user)
	httpUser.Account = &httpproxy.Account{
		Username: account.Username,
		Password: account.Password,
	}
	if err := s.httpServer.AddUser(ctx, httpUser); err != nil {
		_ = s.users.RemoveUser(ctx, user.Email)
		return err
	}
	return nil
}
...
func (s *Server) RemoveUser(ctx context.Context, email string) error {
	if err := s.users.RemoveUser(ctx, email); err != nil {
		return err
	}
	if s.httpServer != nil {
...
func socksAccountFromUser(user *protocol.MemoryUser) (*Account, error) {
	if user == nil {
		return nil, fmt.Errorf("user is nil")
	}
	account, ok := user.Account.(*Account)
	if !ok || account == nil {
		return nil, fmt.Errorf("unsupported socks account type %T", user.Account)
	}
	if account.Username == "" {
		return nil, fmt.Errorf("socks account username is empty")
	}
	return account, nil
}
